...
Uses the official La-Z-Boy .pptx template to create on-brand presentations.
...
def _get_placeholder_indices(slide) -> set[int]:
    """Get available placeholder indices for a slide."""
    return {ph.placeholder_format.idx for ph in slide.placeholders}


def _set_placeholder(slide, idx: int, text: str, available: set[int]) -> None:
    """Set placeholder text if the index exists."""
    if text and idx in available:
...
def _populate_slide(slide, data: dict, layout_key: str) -> None:
    ...
    body = data.get("body", "")
    available = _get_placeholder_indices(slide)

    # Title placeholder (PH0 in most layouts)
    _set_placeholder(slide, 0, title, available)

    # Subtitle (PH1 — title slides only)
    _set_placeholder(slide, 1, subtitle, available)

    # Main body content
    if layout_key in ("content", "content_cream", "content_white", "content_white_full"):
        if body and 1 in available:
    ...
    # Two column
        ...
        _set_placeholder(slide, 1, data.get("left", ""), available)
        _set_placeholder(slide, 2, data.get("right", ""), available)

    # Image + text
    elif layout_key in ("image_text", "image_text_white"):
        image_path = data.get("image", "")
        _set_placeholder(slide, 2, body, available)
        if image_path and os.path.exists(image_path) and 10 in available:
            slide.placeholders[10].insert_picture(image_path)

    # Stats
    elif layout_key == "stats":
        _set_placeholder(slide, 17, body, available)
        stat_pairs = [
        ...
        for value, label, val_ph, lbl_ph in stat_pairs:
            _set_placeholder(slide, val_ph, value, available)
            _set_placeholder(slide, lbl_ph, label, available)

    # Quote
    elif layout_key == "quote":
        _set_placeholder(slide, 10, data.get("quote", body), available)
        _set_placeholder(slide, 2, data.get("attribution", ""), available)
